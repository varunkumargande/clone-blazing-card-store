...
function Checkout() {
  ...
  const [prevAddressBillRadioValid, setPrevAddressBillRadioValid] =
    useState("");
  ...
  const validate = () => {
    ...
    // }
      ...
      let arrayValue = [];
      if (
        !upperPresent(!authValue.isLoggedIn && accountPassword && cpassword)
      ) {
      ...
      if (
        !lowerPresent(!authValue.isLoggedIn && accountPassword && cpassword)
      ) {
        arrayValue.push("Must contain at least 1 Lower Case!");
      }
      if (
        !specialPresent(!authValue.isLoggedIn && accountPassword && cpassword)
      ) {
    ...
    if (
      validateObj.fnameSub &&
      validateObj.fname1Sub &&
      validateObj.numSub &&
      validateObj.num1Sub &&
      validateObj.addressSub &&
      validateObj.postCodeSub &&
      validateObj.postCode1Sub &&
      validateObj.zoneNameSub &&
      validateObj.zoneName1Sub &&
      validateObj.address111Sub &&
      validateObj.countryNameSub &&
      validateObj.countryName1Sub &&
      validateObj.nameSub &&
      validateObj.mailSub &&
      validateObj.methodSub &&
      validateObj.gstNumberSub &&
      validateObj.cpasswordSub &&
      validateObj.prevAddressRadioValidSub &&
      validateObj.prevAddressBillRadioValidSub &&
      validateObj.citySub &&
      validateObj.city1Sub
    ) {
      return true;
    } else {
      return false;
  ...
  const arrayCreate = () => {
    ...
    for (var i = 0; i < len; i++) {
      let vendorids =
        locale[i].vendorId && locale[i].vendorId ? locale[i].vendorId : 0;
            ...
            <div className="custom-checkout-form-contact">
              ...
              <div className="custom-field-row">
                <input
                  ...
                  style={{ borderColor: nameValid && "red" }}
                />
                ...
                {nameValid !== "" && (
                  <span className="custom-field-error" style={{}}>
                    {nameValid}
                  </span>
                )}
              </div>
              ...
              <div className="add-gst-number-container">
                ...
                {gstClick && (
                  <div className="agns-input">
                    <input
                      className="vendor-sign-input"
                      placeholder="GST Number *"
                      name="gstnumber"
                      style={{ borderColor: gstvalid && "red" }}
                      value={gstNumber}
                    ...
                    />
                    {gstvalid !== "" ? (
                      <span className="error-span">{gstvalid}</span>
                    ) : (
                      ""
                    )}
                  </div>
                      ...
                      {cpasswordValid !== "" ? (
                        <>
                          {cpasswordValid.map((error) => (
                            <span className="error-span" style={{}}>
                              {error}
                              <br></br>
                            </span>
                          ))}
              ...
              {authValue.isLoggedIn && addressData.length === 0 ? (
                <>
                  ...
                  {prevAddressRadioValid !== "" ? (
                    <span className="error-span">{prevAddressRadioValid}</span>
                  ) : (
                    ""
                  )}
                  <div className="checkout-add-shipping-address">
                    <Link href="/account/addaddress">
                      <a>+ Add Billing Address</a>
                    </Link>
                  </div>
                </>
              ) : (
                <>
                  {authValue.isLoggedIn && (
                    <>
                      <h2 className="add-select-address">
                        {t("account.ShippingAddress")}{" "}
                        <Link href="/account/addaddress">
                          <a>+ {t("account.AddNewAddress")}</a>
                        </Link>
                      </h2>
                      ...
                      >
                        <input
                          type="checkbox"
                          onClick={(e) =>
                            setBillToSameAddress(e.target.checked)
                          }
              ...
              {!authValue.isLoggedIn && (
                <>
                  ...
                  {!billToSame && (
                    <FormCheckoutBillingInformation
                      cartItems={cartItems}
                      productDetail={details}
                      amount={totalData}
                      addressData={addressData}
          ...
          <div className="custom-checkout-form-right">
            <h2>
              ...
              <Link href="/account/shopping-cart">
                <a className="custom-checkout-form-veiw-cart">
                  {t("view-cart")}
                </a>
              </Link>
            </h2>
            ...
            {cartItems.length > 0 && (
              <div>
                      ...
                      cartItems.map((product) => (
                        <tr>
                          ...
                          <td className="custom-checkout-td-content">
                            ...
                            <a className="checkout-td-prooductname">
                              {product.name}
                            </a>

                            <div className="checkout-td-prooductname-subcontainer">
                              <p>
                                {product.skuName} {product.variantName}
                              </p>
                    ...
                    {couponApplied && (
                      <tr>
                        <td className="custom-td-total">
                          {" "}
                          Discount Amount
                          <p className="coupon-apply-text">
                            Coupons, Vouchers & Promotional
                          </p>
                        </td>
                        <td
                          className="custom-td-total"
                ...
                >
                  <Panel
                    key="1"
                    header={t("checkouts.couponsHeader")}
                    ...
                    <div>
                      ...
                      <div className="apply-coup-new">
                        <input
                          ...
                          style={{ borderColor: couponError && "red" }}
                        />
                        <button
                          style={{ backgroundColor: "#fb641c", width: "81px" }}
                        ...
                        {couponApplied && (
                          <h5 className="apply-coupon-texting">
                            <img src="/static/img/tick-green-new.svg" />
                            Coupon Applied{" "}
                          </h5>
                        )}
                      </div>
                ...
                <div className="payment-new-screen">
                  ...
                  <div
                    ...
                    style={{ width: "calc(100% - 0px)", fontSize: "18px" }}
                  >
                    <Radio.Group
                  ...
                  {paymentvalid !== "" ? (
                    <span className="error-span">{paymentvalid}</span>
                  ) : (
                    ""
                  )}
                </div>

...
function InformationCustom() {
    ...
    useEffect(()=>{
        if(sessionStorage.getItem("spurtUser")) {
            ...
            let lname=JSON.parse(sessionStorage.getItem("spurtUser")).lastName===null?"":JSON.parse(sessionStorage.getItem("spurtUser")).lastName
            ...
            JSON.parse(sessionStorage.getItem("spurtUser")).avatar && setNewDp(imageUrl+"?path="+JSON.parse(sessionStorage.getItem("spurtUser")).avatarPath+"&name="+JSON.parse(sessionStorage.getItem("spurtUser")).avatar+"&width=500&height=500")
        }
    },[])
    ...
    const onChangeHandler=(name,value)=>{
        ...
        const handlePassSubmit=()=>{
            ...
            if(newPass === "" || oldPass==="" || oldPassError !== "" || newPassError.length!==0){
                ...
                changePasswordApi(oldPass,newPass,setPassButtonLoader,setOldPass,setNewPass,setOldPassError,setNewPassError,setPassSubmit)
            }
                                ...
                                <div className="cus-card-row">
                                    <div className="cus-card-account-row" style={{padding:"20px!important"}}>
                                        <div className="cinfo-card-center">
                                            ...
                                            { newDpError===""?<small   style={{marginLeft:"55px",marginTop:"82px"}}>Support (.png .jpg .jpeg) Format & below 2MB Files allowed</small>:<small  className="error-span" style={{marginLeft:"110px",marginTop:"80px"}}>{newDpError}</small>} 
                                            </div>
                                            <div className="cinfo-input-container">
                                                <p>{t('account.FirstName')} *</p>
                                                <input name="fname" placeholder={t('account.FirstName')} maxLength="30" value={fname} onChange={e=>onChangeHandler(e.target.name,e.target.value)}  spellCheck="false" style={{border: fnameError &&"1px solid red"}} />
                                                { fnameError!==""?<span className="error-span">{fnameError}</span>:""} 
                                            </div>
                                            <div className="cinfo-input-container">
                                                <p>{t('account.LastName')} *</p>
                                                <input name="lname" placeholder={t('account.LastName')} maxLength="30" value={lname} onChange={e=>onChangeHandler(e.target.name,e.target.value)} spellCheck="false" style={{border: lnameError &&"1px solid red"}}/>
                                                { lnameError!==""?<span className="error-span">{lnameError}</span>:""} 
                                            </div>
                                            <div className="cinfo-input-container">
                                                <p>Email *</p>
                                                <input name="email"  placeholder="Email" value={email} onChange={e=>onChangeHandler(e.target.name,e.target.value)} spellCheck="false" style={{border: mailValid &&"1px solid red"}}/>
                                                {  mailValid!==""?<span className="error-span">{mailValid}</span>:""}
                                            </div>
                                            <div className="cinfo-input-container">
                                                <p>{t('account.PhoneNumber')} *</p>
                                                <input name="num" maxLength='15' placeholder={t('account.PhoneNumber')} value={num} onChange={e=>onChangeHandler(e.target.name,e.target.value)} spellCheck="false" style={{border: numError &&"1px solid red"}}/>
                                    ...
                                    <div className="cinfo-card-center">
                                        ...
                                        <div className="cinfo-input-container">
                                            ...
                                            <input placeholder={t('account.CurrentPassword')} spellCheck="false" type="password" name="oldPass" value={oldPass} onChange={e=>changeHandlerPassword(e.target.name,e.target.value)} style={{border: oldPassError &&"1px solid red"}}/>
                                            {oldPassError!==""?<span className="error-span">{oldPassError}</span>:""}
                                        </div>
                                        <div className="cinfo-input-container">
                                            <p>{t('account.NewPassword')} *</p>
                                            <input placeholder={t('account.NewPassword')} spellCheck="false"  name="newPass" type="password" value={newPass} onChange={e=>changeHandlerPassword(e.target.name,e.target.value)} style={{border: newPassError.length!==0 && newPassError &&"1px solid red"}}/>

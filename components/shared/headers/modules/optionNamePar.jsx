...
function OptionNameDisplay({ optionName }) {
  ...
  return (
    <p>
      {myArrayOption &&
        myArrayOption.map((name, index) => {
          return (
            <strong key={index}>
              {name.optionArrayName}:{name.optionName}{" "}
            </strong>
          );
        }, [])}
    </p>

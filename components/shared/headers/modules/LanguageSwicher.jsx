...
function LanguageSwicher({ t, i18n }) {
  ...
  useEffect(() => {
    // JSON.stringify
    let lagnchange = JSON.parse(localStorage.getItem("language-blazing"));
    if (lagnchange !== null) {
      // let lagnchange=localStorage.getItem("language-blazing")
      if (lagnchange.code === "eng") {
        dispatch(
          LanguageListLoading(
            imageUrl +
              "?path=" +
              lagnchange.imagePath +
              "&name=" +
              lagnchange.image +
              "&width=20&height=20"
          )
        ...
        setCountryFlag(
          imageUrl +
            "?path=language/&name=Img_1622893818038.png&width=20&height=20"
        );
        ...
        setCountryFlag(
          imageUrl +
            "?path=language/&name=Img_1557569207176.png&width=20&height=20"
        );
      ...
      if (countflagcout && countflagcout.length !== 0) {
        ...
        dispatch(
          LanguageListLoading(
            imageUrl +
              "?path=" +
              countflagcout.at(0).imagePath +
              "&name=" +
              countflagcout.at(0).image +
              "&width=20&height=20"
          )
        );
        // dispatch(LanguageListLoading(imageUrl+'?path=language/&name=Img_1622893818038.png&width=20&height=20'))
  ...
  const onChangeLanguage = (e, datas) => {
    countflagcout &&
      countflagcout.map((equaldata, inex) => {
        if (equaldata.languageId === datas.languageId) {
          ...
          if (datas.code === "eng") {
            ...
            localStorage.setItem("language-blazing", JSON.stringify(datas));
  ...
  return (
    <div className="ps-dropdown language">
      {coutryf && (
        <>
          <a href="#" onClick={(e) => e.preventDefault()}>
            <img src={coutryf && coutryf} alt="martfury" />{" "}
          </a>
        </>
      )}
      ...
      <ul className="ps-dropdown-menu">
        {countflagcout &&
          countflagcout.map((flagcout, index) => (
            <>
              <li>
                <img
                  src={

...
import BlazingPartialSpecification from "../../../../partials/Attributes/PartialSpecification";
...
function DefaultDescription({ ratingInfo, product, forwardedRef }) {
  ...
  const tabChangeScroll = (current) => {
    if (current === "1") {
      ...
      const yOffset = -50;
      const y =
        myDescription.current.getBoundingClientRect().top +
        window.pageYOffset +
        yOffset;
    ...
    if (current === "2") {
      const yOffset = -50;
      const y =
        mySpecification.current.getBoundingClientRect().top +
        window.pageYOffset +
        yOffset;
      window.scrollTo({ top: y, behavior: "smooth" });
    ...
    if (current === "4") {
      const yOffset = -50;
      const y =
        myQuesAndAnswer.current.getBoundingClientRect().top +
        window.pageYOffset +
        yOffset;
      window.scrollTo({ top: y, behavior: "smooth" });
    ...
    <div className="ps-product-desc">
      <div className="ps-product__content ps-tab-root stick-tab-styling">
        <Tabs defaultActiveKey="1" onTabClick={(e) => tabChangeScroll(e)}>
          ...
          <TabPane tab={t("products.Specifications")} key="2">
            <h1></h1>
          </TabPane>
